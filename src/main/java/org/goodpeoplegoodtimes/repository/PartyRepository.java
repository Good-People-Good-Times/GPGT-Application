...
public interface PartyRepository extends JpaRepository <Party, Long> {
    ...
    @Query("select new org.goodpeoplegoodtimes.domain.dto.party.response.PartyListResponseDto" +
            "(p.id, p.title, p.category, p.status, p.createdAt, p.totalPartyMembers, p.currentPartyMembers, p.place) " +
            "from Party p order by p.createdAt desc")
    Page<PartyListResponseDto> fetchPartyList(Pageable pageable);

    @Query("select new org.goodpeoplegoodtimes.domain.dto.party.response.PartyListResponseDto" +
            "(p.id, p.title, p.category, p.status, p.createdAt, p.totalPartyMembers, p.currentPartyMembers, p.place) " +
            "from Party p where p.title LIKE %:cond% order by p.createdAt desc")
    Page<PartyListResponseDto> fetchPartyListBySearch(@Param("cond") String cond, Pageable pageable);

    @Query("select new org.goodpeoplegoodtimes.domain.dto.party.response.PartyListResponseDto" +
            "(p.id, p.title, p.category, p.status, p.createdAt, p.totalPartyMembers, p.currentPartyMembers, p.place) " +
            "from Party p where p.category = :category order by p.createdAt desc")
    ...
    @Query("select new org.goodpeoplegoodtimes.domain.dto.party.response.PartyDetailResponseDto" +
        "(p.id, p.title, p.category, p.status, p.content, p.createdAt, m.id, m.email, m.nickname, m.imgNum, m.role, p.totalPartyMembers, p.currentPartyMembers, p.place) " +
        "from Party p LEFT JOIN p.owner m where p.id = :partyId")
