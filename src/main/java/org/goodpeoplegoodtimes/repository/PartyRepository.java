...
public interface PartyRepository extends JpaRepository <Party, Long> {
    ...
    @Query("select new org.goodpeoplegoodtimes.domain.dto.party.response.PartyDetailResponseDto" +
        "(p.id, p.title, p.category, p.status, p.content, p.owner.nickname, p.createdAt) from Party p where p.id = :id")
    Optional<PartyDetailResponseDto> fetchPartyDetail(@Param("id") Long partyId);
